...
// Sample data for the blog posts (can be moved to a data fetching layer)
const blogs = [
  {
    id: 1,
...
export async function generateMetadata({ params }: IndustryDetailProps): Promise<Metadata> {
  ...
  if (blog) {
    return getMetadata(`/industry-insights/${params.slug}`)
  }
  return getMetadata("/industry-insights") // Fallback metadata
}
  ...
  return (
    <div className="blog-detail py-space">
      <div className="container">
        ...
        <Image src={blog.image || "/placeholder.svg"} alt={blog.title} width={1000} height={600} className="mb-3" />
